...
// Define ThreadPool globally
ThreadPool pool(4);  // Number of threads in the pool, can be adjusted
...
// Handle selection of MST algorithm by the client
...
void handleClient(int clientSocket) {
    ...
    while (isServerRunning) {
        ...
        if (request.find("EXIT") != std::string::npos) {
            ...
            pool.enqueueTask([clientSocket, request, clientGraph]() {
            ...
            pool.enqueueTask([clientSocket, request, &selectedAlgorithm]() {
                handleMSTAlgorithmSelection(clientSocket, request, selectedAlgorithm);
            });
        }
        else if (request.find("PRINT_GRAPH") != std::string::npos) {
            pool.enqueueTask([clientSocket, clientGraph, selectedAlgorithm]() {
